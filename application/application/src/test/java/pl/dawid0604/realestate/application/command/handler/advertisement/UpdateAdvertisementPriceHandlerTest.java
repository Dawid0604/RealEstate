...
class UpdateAdvertisementPriceHandlerTest {
    ...
    void setUp() {
        handler =
                new UpdateAdvertisementPriceHandler(
    ...
    void shouldThrowExceptionWhenUserNotFound() {
        // Given
        final UpdateAdvertisementPriceCommand command = getCommand();
    ...
    void shouldUpdatePrice(final AdvertisementDetails<?> details) {
        // Given
        final UpdateAdvertisementPriceCommand command = getCommand();
        final User foundUser = getDummyUserBuilder().build();
        ...
        verify(foundAdvertisement).updatePrice(priceArgumentCapture.capture());
        verify(eventPublisher).publishEvent(any(AdvertisementPriceChangedEvent.class));

        Assertions.assertThat(advertisementArgumentCaptor.getValue()).isEqualTo(foundAdvertisement);
        Assertions.assertThat(priceArgumentCapture.getValue())
                .satisfies(
                        price -> {
                            Assertions.assertThat(price.value())
                                    .isEqualByComparingTo(command.newPrice());

                            Assertions.assertThat(price.currency()).isEqualTo(MoneyCurrency.PLN);
                        });
    }
